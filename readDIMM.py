"""readDIMM reads in and parses the output from robodimm file.   It will return either a list or dictionary with an entry 
for each column in the file.   It can read in either the seeing.log file or the seeing1t2.log file

Example of usage:


The program can return either a list or dictionary by setting rtype='list' or 'dict'
...
#massdimmseeinglist=['date','time','HR','name','z','n_image', 'FWHN_transverse','FWHM','FWHM_longitude','r0','flux', 'dx', 'dy']
massdimmseeinglist=['date','time','var_s','var_l','seeing','seeing_l','seeing_corr','airmass']
massdimmt1t2list=['HR', 'date','time', 'n_t1', 'FWHM_t1', 'FWHM_transverse_t1', 'FWHM_longitude_t1', 'flux_t1', 'noise_t1', 'scint1_t1', 'scint2_t1', 'strehl1_t1', 'strehl2_t1',\
'n_t2', 'FWHM_t2', 'FWHM_transverse_t2', 'FWHM_longitude_t2', 'flux_t2', 'noise_t2', 'scint1_t2', 'scint2_t2', 'strehl1_t2', 'strehl2_t2', 'extra1', 'extra2']

def lineRead(olist, line):
    """Read in the lines from the file and append them into the list"""
    if line!='#': olist=appendlist(olist, line)
    return olist
...
def makedict(olist, ftype):
    ...
    odict={}
    if ftype=='seeing': objlist=massdimmseeinglist
    elif ftype=='t1t2': objlist=massdimmt1t2list
    else: return olist
...
def readDIMM(dimmfile, ftype='seeing', rtype='list', verbose=False, indatetime=False):
    """readDIMM reads in the output from the robodimm program
       and returns the data in that file depending on what type of file
       it is. ftype is the type of file eitehr 'seeing' or 't1t2'.  'seeing' is the
       default.

       The returned values depend on the options:
        'seeing'-- 'date','time','HR','name','z','n_image', 'FWHN_transverse','FWHM','FWHM_longitude','r0','flux', 'dx', 'dy'
         't1t2' -- 'HR', 'date','time', 'n_t1', 'FWHM_t1', 'FWHM_transverse_t1', 'FWHM_longitude_t1', 'flux_t1', 'noise_t1', 'scint1_t1', 'scint2_t1', 'strehl1_t1', 'strehl2_t1',\
'n_t2', 'FWHM_t2', 'FWHM_transverse_t2', 'FWHM_longitude_t2', 'flux_t2', 'noise_t2', 'scint1_t2', 'scint2_t2', 'strehl1_t2', 'strehl2_t2', 'extra1', 'extra2'
    ...
    if rtype=='dict': olist=makedict(olist, ftype)

    #print out the results if requested
    if verbose:
       if rtype=='list': 
          for i in olist: print i
       if rtype=='dict': 
          for i in olist: print i, olist[i]
